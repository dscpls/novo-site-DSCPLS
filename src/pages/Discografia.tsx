    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {albums.map((al, idx) => (
          ...
          >
            <div className="overflow-hidden bg-[#111] rounded-sm aspect-square relative before:absolute before:inset-0 before:z-10 before:ring-1 before:ring-white/10 before:transition-all group-hover:before:ring-[#00DF59]/60 group-hover:shadow-[0_0_30px_rgba(0,223,89,0.2)] transition-all duration-500">
              <img 
                src={al.cover} 
                alt={al.title} 
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex items-center justify-center">
                <span className="text-[#00DF59] border-2 border-[#00DF59] px-6 py-2 text-xs tracking-widest bg-black/80 font-bold backdrop-blur-sm shadow-[0_0_15px_rgba(0,223,89,0.4)] uppercase">
                  {t('disc.listen')}
                </span>
              </div>
            </div>
            <div className="mt-6 flex flex-col items-start">
              <span className="text-[10px] tracking-[3px] text-[#FFE600] font-mono mb-2 border-b border-white/10 pb-1">{al.type} // {al.year}</span>
              <h3 className="text-lg font-bold text-white group-hover:text-[#00DF59] transition-colors">{al.title}</h3>
            </div>
          </motion.a>
        ))}
      </div>
